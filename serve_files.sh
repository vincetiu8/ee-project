...
mx=1000; my=1000; nframes=500; dd if=/dev/urandom bs="$((mx*my*3))" count="$nframes" | avconv -r 25 -s "${mx}x${my}" -f rawvideo -pix_fmt rgb24 -i - 1000px.mp4
mkdir 10F
for i in {0..9}
do
  head -c 100M </dev/urandom > 10F/100M"$i"
done
echo "10F 1M 10M 100M 1G 10000px.png 1000px.mp4" > files.txt
until ipfs add 1M
do
  sleep 1
done
